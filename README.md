...
- **🎯 プライベート CodingRoom**: `/coding start` でユーザー専用のプライベートチャンネルを自動生成。
- **🤖 マルチモデル対応**: OpenRouter 経由で最新モデル (Claude 3.5 Sonnet 等) を利用可能。
- **🔐 セキュア設計**: ユーザーの API キーは AES-256 で暗号化して保存。
- **📁 ファイル管理**: セッション内でのファイル保存、一覧表示、ZIP ダウンロードに対応。
...
| コマンド | 説明 |
|:---|:---|
| `/api-key register` | API キーの登録 (暗号化保存) |
| `/coding start` | セッション開始 (プライベートチャンネル作成) |
| `/coding chat` | AI との対話 (CodingRoom 内) |
| `/coding end` | セッション終了 (チャンネル削除) |
| `/setting` | ユーザー設定・モデル変更・利用状況確認 |
| `/download` | プロジェクトの ZIP ダウンロード |
